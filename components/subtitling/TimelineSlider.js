...
const TimelineSlider = ({ videoRef, hiddenVideoRef }) => {
    ...
    const capture = () => {
      if (currentTime <= duration && captures.length < 20) {
        ...
        if (currentTime <= duration && captures.length < 20) {
          ...
          canvasRefs.current = captures.map(
            (_, i) => canvasRefs.current[i] || React.createRef()
          );
        }
      }
    };
  ...
  useEffect(() => {
    ...
    return () => {
      if (hiddenVideoRef.current) {
        hiddenVideoRef.current.removeEventListener(
          'loadeddata',
          captureThumbnails
        );
      ...
      <div className={styles.timeline_container}>
        <div className="flex justify-center">
          <div className="flex flex-row items-center">
            <div
              className={`mr-s1 h-[24px] w-[24px] rounded-full bg-white ${
                isPlaying ? '' : 'pl-[2px] pt-[1px]'
              }`}
            >
              <Image
                src={isPlaying ? pause_black : play_black}
                alt=""
                width={24}
                height={24}
            ...
            <div className={styles.thumbnail_container}>
              {canvases.map((_, index) => (
                <div
                  ...
                  className="w-5% h-[56px] overflow-hidden"
                ></div>
              ))}
            </div>
          </div>
