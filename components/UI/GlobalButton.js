...
const GlobalButton = ({
  ...
  const buttonContent = (
    <div
      className={`
        ...
        transition-300 relative block cursor-pointer rounded-full pb-s1 pt-s1.5 text-lg font-medium ${extraClasses} ${
  ...
  if (purpose === 'route') {
    button = <Link href={route}>{buttonContent}</Link>;
    ...
    button = (
      <div onClick={isLoading ? () => null : onClick}>{buttonContent}</div>
    );
